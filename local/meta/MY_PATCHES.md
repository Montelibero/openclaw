...
- `feat/*`, `chore/*`, `fix/*` = thematic source branches, atomic and rebaseable onto upstream.
- `local/meta` = meta-ветка: `BRANCHES.md` (реестр веток + recipe), этот файл, `fork-overlay-workflow.md`, сгенерированные config/inventory артефакты.
- `deploy` = `main` + merged `feat/*`/`fix/*`/`chore/*` + merged `local/meta`; daily-driver, source for prod docker builds.

Update this file on `local/meta` whenever a branch lands on `deploy` (or gets dropped because upstream absorbed it), and add the branch to the `local/meta/BRANCHES.md` registry.

Overlay workflow status:

- Схема и rationale — `fork-overlay-workflow.md` рядом; реестр активных веток и recipe пересборки — `BRANCHES.md` рядом.
...
Rebuild flow (rebase веток, пересборка `deploy`, реген-проверка) — см. Recipe в `BRANCHES.md`.
...
**Status:** in-prod (с 2026-09-13)
**Why:** файловый менеджер в dashboard: browse/download/upload/archive. Собственный ванильный рантайм — чистый Node.js + vanilla JS UI, **нулевых зависимостей**. CloudCmd (и Express/Socket.IO за ним) полностью выпилен.

**Changes:**

- `extensions/files-manager/` — bundled plugin. Регистрирует `/files` (static UI + REST API), Gateway method `files-manager.controlUiSession` выдаёт one-time ticket; plugin exchange делает HttpOnly `SameSite=Strict` cookie для `/files`.
- `extensions/files-manager/src/api.ts` — файловые операции (list/read/write/upload/download/archive/delete); архивы — **tar.gz**, не zip (в slim-образе нет zip).
- `extensions/files-manager/src/web-html.ts`, `src/web/index.html` — ванильный single-file UI, без фреймворков.
...
- `ui/src/styles/layout.css` — sidebar scrollbar + nav max-height (длинный список plugin-вкладок).
...
**Build:** Personal Docker CI передаёт `OPENCLAW_EXTENSIONS=files-manager`, поэтому plugin dist запекается в image; runtime install не нужен. Внешних npm-зависимостей у плагина нет, lockfile ветка не меняет (net-zero).

**History note:** vanilla-рефактор (`162b3c3c834`), фикс сайдбара (`fb763abfe5a`) и tar.gz-фикс (`a05bdf4c81c`) изначально были закоммичены напрямую в `deploy` поверх merge этой ветки. Потом они сложены обратно в ветку cherry-pick'ом — иначе следующий rebuild (reset --hard main + merge веток) молча вернул бы CloudCmd. Регенерация plugin inventory живёт на `chore/my-patches`.

**Verification:** содержимое ветки байт-в-байт совпадает с продовым `deploy` по всей поверхности плагина и UI: `git diff feat/files-manager deploy -- extensions/files-manager ui/src packages/gateway-protocol src/gateway/control-ui-plugin-tabs.* tsdown.config.ts ...` пуст. Образ `ghcr.io/montelibero/openclaw:latest` с этого кода работает в проде с 2026-09-13. Исторический CloudCmd-вариант (`plugins:inventory:check`, targeted Vitest `extension-misc` 4/4) — заменён; при следующих правках прогонять `pnpm test extensions/files-manager`.
